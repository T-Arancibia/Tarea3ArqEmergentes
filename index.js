...
app.get('/', (req, res) => {
    res.send("API IoT en Funcionamiento");
});
...
app.delete('/api/v1/company/delete', (req, res) => { //DELETE
    ...
    db.all(query, (error, rows) => {
        ...
        console.error(error);
            ...
            if(rows.length >= 1){
                const query1 = `DELETE FROM Company WHERE company_api_key = '${req.body.company_api_key}';`;
                db.run(query1, (error) => {
                    if (error) {
                        console.error(error);
                    } 
                    else {
                        console.log(`Company ${req.body.company_api_key} deleted`);
                        res.status(200).json({status: "successful"});
                    }
                });
            }
            else{
                console.log(`Error, la compañia de api key ${req.body.company_api_key} no existe`);
                res.status(400).json({status: "failed", description: `the company api key ${req.body.company_api_key} not exist`});
            }
        }
    });
});
...
app.put('/api/v1/company/edit', (req, res) => { //UPDATE
    ...
    db.all(query, (error, rows) => {
        ...
        console.error(error);
            ...
            if(rows.length >= 1){
                if(rows[0].company_name == req.body.new_company_name){
                ...
                db.all(query1, (error, rows) => {
                   if(error) {
                    ...
                    if (error) {
                        ...
                        console.log(`Error, combinacion Company con id: ${ID} y Location con id: ${req.body.location_id} no arroja coincidencias`);
                        res.status(400).json({status: "failed", description: `combinacion company api key y location id: ${req.body.location_id} no arroja coincidencias`});
                    }
                   }
                })
    ...
    db.all(query, (error, rows) => {
        ...
        console.error(error);
            ...
            if(rows.length >= 1 ){
                ...
                db.all(query1, (error, rows) => {
                   if(error) {
                    ...
                    if(rows.length >=1) {
                        ...
                        console.log(`Error, combinacion Company con id: ${ID} y Location con id: ${req.body.location_id} no arroja coincidencias`);
                        res.status(400).json({status: "failed", description: `combinacion Company api key ${req.params.company_api_key} y Location con id: ${req.body.location_id} no arroja coincidencias`});
                    }
                   }
                })
            }
                ...
                console.log(`Error, ${req.body.company_api_key} no esta asociada a ninguna company`);
                res.status(400).json({status: "failed", description: `${req.params.company_api_key} no esta asociada a ninguna company`});
            }
        }
    });
});
